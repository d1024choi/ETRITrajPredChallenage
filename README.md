...
## Submission
Given masked test data (scene files without the positions and headings of the agents over future horizon), predict K=6 possible future trajectories for each agent in a scene. The prediction results must be stored following the method described in 'prediction_results_submission.py'. Zip the pickle files generated by the method and submit the zip file after naming it as 'prediction_result_{participants number}.zip' (e.g., prediction_result_1024.zip).
